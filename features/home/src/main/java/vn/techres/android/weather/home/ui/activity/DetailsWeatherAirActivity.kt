...
class DetailsWeatherAirActivity : AppActivity() {
    private lateinit var binding: ActivityDetailsWeatherAirBinding
    ...
    override fun getLayoutView(): View {
        binding = ActivityDetailsWeatherAirBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun initData() {
        //
        locationName = intent.getStringExtra(AppConstants.DETAILS_LOCATION_FIND_NAME).toString()
        locationLat = intent.getDoubleExtra(AppConstants.DETAILS_LOCATION_FIND_LAT, 0.0)
        locationLon = intent.getDoubleExtra(AppConstants.DETAILS_LOCATION_FIND_LON, 0.0)
        if (locationName != "" && locationLat != 0.0 && locationLon != 0.0) {
            setupData(locationName, locationLat, locationLon)
    ...
    private fun setupView() {
        adapterWeatherAirHours= ListHoursWeatherAirAdapter(this)
        adapterWeatherAirHours.setData(listHoursAirWeather)
        AppUtils.initRecyclerViewHorizontal(binding.ilWeatherAirHours.rcvListWeatherHours,adapterWeatherAirHours)

        adapterWeatherDays = ListDaysSevenAdapter(this)
        ...
        binding.imvHome.clickWithDebounce(500){
            ...
            .request(object : HttpCallback<WeatherDays>(this) {
                ...
                override fun onSucceed(result: WeatherDays) {
                    ...
                    if (result.cod == 200) {
                        ...
                        binding.ilWeatherOther.tvThereMayBeRain.text=result.list[0].pop.toString()+getString(R.string.persent)
                    } else {
                        toast(getString(R.string.load_fail))
                    }
                }
        ...
        EasyHttp.get(this).api(GetListAirHours.param(lat, lon))
            .request(object : HttpCallback<AirNow>(this){
                override fun onSucceed(result: AirNow) {
                    ...
                    EasyHttp.get(this@DetailsWeatherAirActivity).api(GetListWeatherHours.params(lat, lon,AppUtils.returnUnits(2))).request(
                        object :HttpCallback<WeatherDays>(this@DetailsWeatherAirActivity){
                            ...
                            override fun onSucceed(result: WeatherDays) {
                                ...
                                if (result.cod ==200){
                                    ...
                                    Timber.tag("listSize11").d("${listHoursWeather.size},${listHoursAir.size}")
                                    ...
                                    if (listHoursAir.size > 0 && listHoursWeather.size > 0){
                                        for (i in 0 until 25) {
                                            val weather = listHoursWeather[i] // Nhận dữ liệu thời tiết tại chỉ mục i
                                            val air = listHoursAir[i] // Nhận dữ liệu không khí tại chỉ số i
                                            // Tạo một đối tượng mới kết hợp dữ liệu thời tiết và không khí
                                            val airWeather = ListAirWeatherHours(i.toLong(),air,weather)
                                        ...
                                        listHoursAirWeather.mapIndexed { i, data ->
                                            numMapQuarter[i]=AppUtils.getDayDetailsHours(data.listWeather.dt,true)
                                        }
                                        if (listHoursAirWeather.size > 0) {
                                            setDataReportTemperatureAQI(
                                                listHoursAirWeather,
                                                numMapQuarter,true,binding.ilLineChartTemperature.lineChartAQI
                                            )
                                            setDataReportTemperatureAQI(
                                                listHoursAirWeather,
                                                numMapQuarter,false,binding.ilLineChartTemperature.lineChartTemperature
                                            )
                                        }
    ...
    private fun setDataReportTemperatureAQI(
        data: ArrayList<ListAirWeatherHours>, numMapQuarter: HashMap<Int, String>,isAQI:Boolean,lineChart: LineChart
    ) {
        ...
        if (data.size < 1) {
            ...
            if (isAQI) binding.ilLineChartTemperature.lineChartAQI.data = generateLineDataTemperatureAQI(data,true)
            else binding.ilLineChartTemperature.lineChartTemperature.data = generateLineDataTemperatureAQI(data,false)
        }
    ...
    private fun initLineChart(chart: LineChart, numMapQuarter: HashMap<Int, String>) {
        ...
        // set an alternative background color
        chart.setBackgroundColor(getContext().getColor(R.color.gray_200_50))
        chart.isDoubleTapToZoomEnabled = true
        val custom: ValueFormatter = MyAxisValueFormatter()
        val leftAxis: YAxis = chart.axisLeft
        leftAxis.typeface = TypeFaceUtils.getRobotoRegularTypeface(getContext())
        leftAxis.setLabelCount(10, false)
        leftAxis.spaceTop = 15f
        leftAxis.valueFormatter = custom
        val xAxis: XAxis = chart.xAxis
        xAxis.position = XAxis.XAxisPosition.BOTTOM
        xAxis.typeface = TypeFaceUtils.getRobotoRegularTypeface(getContext())
        ...
        val customMarkerView = CustomLineMarkerView(
            getContext(), R.layout.layout_line_marker
        )
        chart.marker = customMarkerView
        chart.animateY(1500)
        chart.setNoDataTextColor(getContext().getColor(R.color.text_color_blue))
        chart.setNoDataText(getContext().getString(R.string.no_data))
        chart.setPinchZoom(true)
        chart.fitScreen()
    }
    ...
    private fun generateLineDataTemperatureAQI(
        ...
        val lineTemperature = LineDataSet(entriesTemperature, getString(R.string.map_chart_temperature))
        lineTemperature.color =
            getContext().getColor(R.color.blue_700)
        lineTemperature.setCircleColor(getContext().getColor(R.color.blue_700))
        ...
        lineTemperature.fillColor = ColorTemplate.colorWithAlpha(
            getContext().getColor(R.color.blue_700), 200
        )
        ...
        lineAQI.color =
            getContext().getColor(R.color.green_600)
        lineAQI.setCircleColor(getContext().getColor(R.color.green_600))
        ...
        lineAQI.fillColor = ColorTemplate.colorWithAlpha(
            getContext().getColor(R.color.green_600), 200
        )
